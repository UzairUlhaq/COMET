...
root_task_name='processed_data_dirs/lnpdb_heartkidney_gen/fold_V0'  # data folder name
root_save_dir='./save_lnpdb_heartkidney'  # replace with your save path
root_tmp_save_dir = './tmp_save_lnpdb_heartkidney'
...
full_dataset_task_schema_path = "task_schemas/lnpdb_heart_kidney_schema.json"
                                                                        ...
                                                                        for percent_noise_type in percent_noise_types:
                                                                            for patience in patiences:
                                                                                for train_data_ratio in train_data_ratios:
                                                                                    ...
                                                                                    exp_name=f'lnpdb_heartkidney_fold_V0_lnp_{loss_func}-bs{batch_size}-lr{lr}-lnpmodparams{lnp_encoder_layers}-{lnp_encoder_embed_dim}-{lnp_encoder_ffn_embed_dim}-{lnp_encoder_attention_heads}-trainrat{train_data_ratio}-ep{max_epoch}-pat{patience}-metric{metric}-cagrad{cagrad_c}-percentnoise{percent_noise}-labelmargin{contrast_margin_coeff}-seed{seed}'
                                                                                    ...
                                                                                    train_result = subprocess.run(f"python ../unimol/train_np.py {data_path} --task-name {task_name} --user-dir ../unimol --train-subset train --valid-subset valid \
                                                                                        --conf-size {conf_size} \
                                                                                        ...
                                                                                        --lr-scheduler polynomial_decay --lr {lr} --warmup-ratio {warmup} --max-epoch {max_epoch} --batch-size {local_batch_size} --required-batch-size-multiple 1 --pooler-dropout {dropout} \
                                                                                    ...
                                                                                    if train_result.returncode != 0 or not os.path.exists(eval_weight_path):
                                                                                        print("Training failed or checkpoint_best.pt was not created. Skipping inference for: ", exp_name)
                                                                                        continue
                                                                                    ...
                                                                                    subprocess.run(f"python ../unimol/infer_np.py --user-dir ../unimol {data_path} --task-name {task_name} --valid-subset test \
                                                                                        --num-workers {num_workers} --ddp-backend=c10d --batch-size {eval_batch_size} --required-batch-size-multiple 1 \
                                                                                        --task mol_np_finetune --loss {loss_func} --arch np_unimol \
                                                                                        --classification-head-name {task_name} --num-classes {task_num} \
